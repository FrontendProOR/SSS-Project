...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8080/api/korisnici/registracijaadmina', formData, {
        headers: {
          'authorization': token 
        }
      });
  ...
  return (
    <div className="container mt-5">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" className="btn btn-primary">Registruj Admina</button>
      </form>
    </div>
  );
};
